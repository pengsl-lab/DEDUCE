...
from models.cancer_cluster_attention import Transformer, ED4
...
def draw_fig(list, cancertype, epoch):
    ...
    print(x1)
    y1 = list
    if not os.path.exists('./results/cancer/{}'.format(cancertype)):
        os.makedirs('./results/cancer/{}'.format(cancertype))
        # open('./results/{}/{}'.format(datatypes, typenumbers), 'w')
    # np.savetxt('./results/{}/{}/Train_loss.png'.format(datatypes, typenumbers), 'w')
    save_file = './results/cancer/{}/Train_loss.png'.format(cancertype)
    plt.cla()
...
if __name__ == '__main__':
    ...
    if args.Train == True:
        ...
        for datapath, resultpath in zip(data_dir_list, result_dir_list):
            ...
            traindata = DataLoader(traindata, batch_size=256, shuffle=False, num_workers=8, pin_memory=True)


            # device = 'cuda' if torch.cuda.is_available() else 'cpu'
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            # 模型的存储位置
            # a = os.path.dirname(resultpath)
            # print(a)
            b = os.path.basename(resultpath)
            model_path = './save/{}/{}'.format("cancer", b)
            print(model_path)
            ...
            if args.model == 'AE':
                ...
                model = Transformer(ED4(input_dim =omics.shape[1], out_dim=20), args.feature_dim, args.typenumbers)
        ...
        for datapath, resultpath in zip(data_dir_list, result_dir_list):
            ...
            print('*' * 100)
            traindata = torch.from_numpy(omics)
            traindata = DataLoader(traindata, batch_size=256, shuffle=True, num_workers=8, pin_memory=True)
            ...
            X, h = inference(traindata, model, device)
            ...
            np.savetxt('./results/cancer/{}/{}_label.txt'.format(b, args.model), X, fmt = '%d')

            np.savetxt('./results/cancer/{}/{}_10.txt'.format(b, args.model), h)
            ...
            encoded_factors = h
            resultpath = './results/cancer/{}'.format(b)
            datapath = 'data/cancer/{}/{}'.format(args.datatypes, args.typenumbers)
